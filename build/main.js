...
const simul = (() => {
    ...
    const aisleSpace = (corridorSize - columnEdgePad * 2 - rowSizePx * colSetCnt) / (colSetCnt - 1);
    const seats = Array(colSetCnt * rowCnt * colCnt)
        .fill(0)
        .map((_, i) => {
        ...
        const colPosition = column * (rowSizePx + aisleSpace) + rowOffset * (seatSize + seatGap);
        return {
            id: String(i),
            x: canvas.width -
                seatSize -
                ...
                seatSize / 2 -
                seatOffset,
            y: colPosition + columnEdgePad - corridorSize / 2 + seatSize / 2 - seatOffset,
        };
    });
...
const art = (() => {
    ...
    const planeBody = ctx.createLinearGradient(0, -simul.corridorSize / 2 - 30, 0, +simul.corridorSize / 2 + 30);
...
function clusterSeating(seatCluster = 1, reversed = false) {
    ...
    if (reversed)
        fullSeats.reverse();
    let clusterIndex = 0;
    let seatBag = fullSeats.slice(clusterIndex * seatCluster, (clusterIndex + 1) * seatCluster);
    for (const pass of simul.passengers) {
        ...
        if (!seatBag.length) {
            // Build next cluster
            seatBag = fullSeats.slice(++clusterIndex * seatCluster, (clusterIndex + 1) * seatCluster);
        }
    }
}
function WindowMiddleAisleSeating(strict = false) {
    if (simul.colSetCnt !== 2)
        throw Error("Only works in 2-column seatings!");
    ...
    function getColumnSet() {
        ...
        for (let i = 0; i < simul.colCnt; i++) {
            ...
            indexBag.push(
            // Go outside-in
            bigRowIndex + rowOffset, bigRowIndex + simul.rowCnt * 2 - 1 - rowOffset);
        }
        if (strict) {
...
const dummyPassenger = Object.freeze({
    x: Infinity,
    y: Infinity,
    stowTime: -simul.stowFinishWait,
    seat: Object.freeze({ x: Infinity, y: Infinity }),
});
function findClosestPassenger(comparator, currentIndex) {
...
(() => {
    const restartBtn = HTML.getOne(".restart");
    const fastForwardBtn = HTML.getOne(".ffd");
    const seatingSelection = HTML.getOne(".seating");
...
function draw() {
    ...
    for (const { x, y } of simul.seats) {
        ...
        ctx.beginPath();
        ctx.roundRect(x - simul.seatSize / 2 + ctx.lineWidth / 2 + simul.seatOffset, y - simul.seatSize / 2 + ctx.lineWidth / 2 + simul.seatOffset, trueSeatSize, trueSeatSize, simul.seatSize * 0.05);
        ctx.fill();
        ctx.stroke();
        ctx.beginPath();
        ctx.roundRect(x +
            ...
            simul.seatSize * 0.1 -
            ctx.lineWidth +
            simul.seatOffset, y - simul.seatSize / 2 + simul.seatOffset, simul.seatSize * 0.2, simul.seatSize * 0.8, simul.seatSize * 0.01);
        ctx.fill();
        ctx.stroke();
    }
    // Passenger
